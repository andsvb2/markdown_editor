...
class Test(StageTest):
            ...
            with open("output.md", "r") as outfile:
                ...
                if output != self.answers[attach]:
                    return CheckResult.wrong(
                        "The result written to the output file is wrong."
                    )
        except IOError:
    ...
    def test1(self):
        ...
        if "level" not in output:
            return CheckResult.wrong(
                'Header formatter should prompt a user for both level and text, i.e "- Level: > "'
            )

        output = pr.execute("4").strip().lower()
        if "text" not in output.strip().lower():
            return CheckResult.wrong(
                'Header formatter should prompt a user for both level and text, i.e "- Text: > "'
            )

        output = list(
            map(lambda item: item.lower(), pr.execute("Hello World!").split("\n"))
        )
        if len(output) != 3:
            return CheckResult.wrong(
                "Please remember that header formatter switches to a new line automatically"
            )
        ...
        if output[1]:
            return CheckResult.wrong(
                "Please check whether some redundant data is printed after a header"
            )
    ...
    def test2(self):
        ...
        output = list(
            map(lambda item: item.lower(), pr.execute("plain text").split("\n"))
        )
        if len(output) != 2:
            return CheckResult.wrong(
        ...
        output = list(
            map(lambda item: item.lower(), pr.execute("bold text").split("\n"))
        )
        if len(output) != 2:
            return CheckResult.wrong(
    ...
    def test3(self):
        ...
        output = list(
            map(lambda item: item.lower(), pr.execute("italic text").split("\n"))
        )
        ...
        output = list(
            map(lambda item: item.lower(), pr.execute("code.work()").split("\n"))
        )
        if len(output) != 2:
            return CheckResult.wrong(
    ...
    def test4(self):
        ...
        output = list(
            map(
                ...
                pr.execute("https://www.google.com").split("\n"),
            )
        )
        if len(output) != 2:
            return CheckResult.wrong(
        ...
        output = list(
            map(lambda item: item.lower(), pr.execute("new-line").split("\n"))
        )
    ...
    def test5(self):
        ...
        if len(output) != 6:
            return CheckResult.wrong(
                "Ordered list formatter should switch to a new line automatically"
            )

        if (
            output[0] != "1. first"
            or output[1] != "2. second"
            or output[2] != "3. third"
            or output[3] != "4. fourth"
        ):
            return CheckResult.wrong(
